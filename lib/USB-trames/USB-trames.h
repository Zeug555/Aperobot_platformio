...
#define Quantite 6    // déplacement en mm de chaque moteur       'd'
byte typeOrdre;
...
{ // cette fonction envoie la trame vers RPI de la position d'un actionneur demandé
  ...
  tamponEmis[1] = 'E';
  if (grappe == 0)
    tamponEmis[2] = actionneur + '0';
  else
    tamponEmis[2] = grappe - 1 + 'A';
  if (val == 'X')
    tamponEmis[3] = val;
  else
    tamponEmis[3] = val + '0';
...
{ // cette fonction construit et envoie la trame d'état d'une séquence
  ...
  case MonteTribune:
    tamponEmis[1] = 'C';
    if (numEtape <= 9)
      tamponEmis[2] = '2'; // si numEtape <= 9 '2'
    else
      tamponEmis[2] = '3'; // sinon '3
    break;
  }
...
void bougeEnQuantite()
{ // cette fonction doit traduire les ordres de déplacement de la trame recue en 3 ordres sur le bus RS485
  // rappel : La commande de chaque moteur se fait par
  // MouveRelPulse(uint8_t adr_moteur, uint8_t sens, uint16_t vitesse, uint8_t acceleration, uint32_t impulsions)
  uint32_t qte1 = enVal(3, 3) * impulsions_mm;
  byte vitM1 = enVal(6, 2);
  uint32_t qte2 = enVal(9, 3) * impulsions_mm;
  byte vitM2 = enVal(12, 2);
  uint32_t qte3 = enVal(15, 3) * impulsions_mm;
  byte vitM3 = enVal(18, 2);
  byte sensM1 = tamponRecep[2] - '0';
  byte sensM2 = tamponRecep[8] - '0';
  byte sensM3 = tamponRecep[14] - '0';
  MouvRelPulse(mot_gauche, sensM3, vitM3, 0, qte3);
  MouvRelPulse(mot_droit, sensM2, vitM2, 0, qte2);
  MouvRelPulse(mot_arriere, sensM1, vitM1, 0, qte1);
  Serial.println("Déplacement en quantité de mm");
}
...
{ // cette fonction analyse la trame d'un ordre de mouvement en vitesse
  ...
  if (valN(2, 3) == false)
    ok = false;
  if ((tamponRecep[5] != '0') && (tamponRecep[5] != '1'))
    ok = false;
  if (valN(6, 3) == false)
    ok = false;
  if ((tamponRecep[9] != '0') && (tamponRecep[9] != '1'))
    ok = false;
  if (valN(10, 3) == false)
    ok = false;
  if ((tamponRecep[13] != '0') && (tamponRecep[13] != '1'))
    ok = false;
  return (ok);
}

bool testDeplacement()
{ // cette fonction analyse la trame d'un ordre de mouvement en deplacement
  // rappel de la trame : [Stx] [Cde] [M1s] [M1ddd] [VM1] [M2s] [M2ddd] [VM2] [M3s] [M3ddd] [VM3] [CRC] [Etx]
  bool ok = true;
  if ((tamponRecep[2] != '0') && (tamponRecep[2] != '1'))
    ok = false;
  if (valN(3, 3) == false)
    ok = false;
  if (valN(6, 2) == false)
    ok = false;
  if ((tamponRecep[8] != '0') && (tamponRecep[8] != '1'))
    ok = false;
  if (valN(9, 3) == false)
    ok = false;
  if (valN(12, 2) == false)
    ok = false;
  if ((tamponRecep[14] != '0') && (tamponRecep[14] != '1'))
    ok = false;
  if (valN(15, 3) == false)
    ok = false;
  if (valN(18, 2) == false)
...
{ // reconnait le type d'ordre, retournfaux sinon
  ...
  case 'd':
    if (DEBUGUSB)
      Serial.print("Ordre de Déplacement embase");
    ok = true;
    typeOrdre = Quantite;
    break;
...
{ // la trame reçue est compréhensible, il faut la traiter !
  ...
  case Deplace:
    ...
    break;
  case Quantite:
    bougeEnQuantite(); // envoi aux moteurs les consignas en distances
    break;
  case OTOS:
...
byte calculCRC(String chaine)
{
  ...
  crc |= 0x80;
  if (DEBUGCRC)
    Serial.printf("crc calculé = %d ", crc);
  return (crc);
}
